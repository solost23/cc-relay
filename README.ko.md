...
| 저위험, 유효 가중치 < 4 | 자동 승인 (기준선 불필요) |
| 저위험, 유효 가중치 ≥ 4, 승인율 ≥ 90% | 자동 승인 |
| 중위험, 유효 가중치 < 7 | 인터럽트하여 기준선 구축 |
| 중위험, 유효 가중치 ≥ 7, 승인율 ≥ 85% | 자동 승인 |
| 그 외 | 인터럽트 |

승인율은 **지수 시간 감쇠**(반감기: 7일)를 사용합니다 — 최근 결정이 오래된 기록보다 높은 가중치를 가집니다. 이전에 항상 승인하던 작업을 거부하기 시작하면, 가중 승인율이 며칠 내에 빠르게 떨어지고 Relay가 다시 인터럽트하기 시작합니다. 오래된 승인 기록은 자연스럽게 감쇠하므로 시스템이 영구적으로 자동 승인 상태에 고정되지 않습니다.
