...
**解決する問題：** Claude Code がタスク実行中に権限確認が必要になったとき、あなたがターミナルの前にいなければ、そのまま無音で止まってしまいます。戻ってきたらタスクがずっと止まっていた、という経験があるはずです。Relay はタスクが一時停止した瞬間にデスクトップ通知を送るので、判断が必要な瞬間を見逃しません。
